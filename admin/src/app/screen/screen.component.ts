...
export class ScreenComponent {
 ...
 index: number =0;
    ...
    isValid(screenName:string, NoScreenId:string, location:string, resWidth:string, resHeight:string, Width:string, height:string, loop:string, NoOFscreens:string, latitude:string, longitude:string, mediaType:string, environment:string, sector:string, imagePaths:string ): boolean {
        this.isError = false;
		this.errorMessage = "";
		if(this.screenName == "") {
			this.isError = true;
    ...
    onCreate(modal: any) {
        if(!this.isValid(this.screenName, this.NoScreenId,  this.location, this.resWidth, this.resHeight, this.width, this.height, this.loop, this.NoOFscreens, this.latitude, this.longitude, this.mediaType, this.environment, this.sector,this.imagePaths)) {
            return;
        }
        ...
        this.selectedFiles.forEach((file, index) => {
            // formData.append('images', file, file.name);
             formData.append("imagePaths", file , `screenImage_${index}_${file.name}`);
