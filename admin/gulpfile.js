...
gulp.task('dark', function(){
   return gulp.src('src/assets/css/style-dark.scss')
        .pipe(sourcemaps.init())
        .pipe(sass())
		.pipe(beautify.css({ indent_size: 4 }))
        .pipe(sourcemaps.write('.'))
...
gulp.task('transparent', function(){
	return gulp.src('src/assets/css/style-transparent.scss')
		 .pipe(sourcemaps.init())
		 .pipe(sass())
		 .pipe(beautify.css({ indent_size: 4 }))
		 .pipe(sourcemaps.write('.'))
...
gulp.task('skin', function(){
 ...
 gulp.task('default', gulp.series('watch', 'dark', 'transparent', 'skin'))
